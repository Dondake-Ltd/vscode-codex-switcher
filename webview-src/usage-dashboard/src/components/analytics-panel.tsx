...
import type { UsageDashboardReadyState, UsageHistorySample, UsagePanelProfile } from '../../../../src/usageDashboardTypes';
import {
  buildUsageHistoryPointList,
  buildUsageHistorySparklinePath,
  ...
  formatTimestamp,
  formatTokenUsage,
  formatUsageHistoryTooltip,
  getDisplayPercentValue,
  getHistoryRangeLabel,
  getPeakUsageValue,
  getSeverityTone,
  getTimeProgressPercent,
  getUsageHistoryContextLabel,
  getUsageHistoryPointX,
  getUsageHistorySamplesForRange,
  ...
  return (
    <section class="analytics-panel__section">
      <div class="analytics-panel__section-head">
        ...
        <span class={`badge badge--${tone}`}>{formatDisplayPercent(descriptor.window, state.percentDisplayMode)} {state.percentDisplaySuffixCompact}</span>
      </div>
      ...
      <div class="analytics-row">
        ...
        <strong>{formatPercentValue(timePercent)}</strong>
      </div>
      <div class="analytics-panel__detail">Reset {formatResetLong(descriptor.window.resetsAt)}</div>
    </section>
  );
}

function HistoryChart(props: { state: UsageDashboardReadyState; samples: UsageHistorySample[] }) {
  ...
  const startMs = parseIsoMs(props.samples[0]?.recordedAt);
  const endMs = parseIsoMs(props.samples[props.samples.length - 1]?.recordedAt);
  const fiveHourPoints = buildUsageHistoryPointList(props.samples, 'primaryUsedPercent');
  const weeklyPoints = buildUsageHistoryPointList(props.samples, 'secondaryUsedPercent');
  const series = [
    fiveHourPoints.length
      ? {
          ...
          peak: getPeakUsageValue(props.samples, 'primaryUsedPercent'),
    ...
    weeklyPoints.length
      ? {
          ...
          peak: getPeakUsageValue(props.samples, 'secondaryUsedPercent'),
    ...
    <div class="history-card">
      <div class="history-card__stats">
        <div class="history-stat">
          <span>Range</span>
          <strong>{getHistoryRangeLabel(props.state.historyRange)}</strong>
        </div>
        <div class="history-stat">
          <span>Samples</span>
        ...
        {series.map((entry) => (
          <div class="history-stat" key={entry.key}>
            <span>Peak {entry.label}</span>
            <strong>{entry.peak !== undefined ? formatPercentValue(entry.peak) : 'Unknown'}</strong>
          </div>
        ))}
      </div>
      <div class="history-chart">
        <svg viewBox={`0 0 ${svgWidth} ${svgHeight}`} width="100%" height="232" role="img" aria-label="Usage history chart">
          <g transform={`translate(${chartOffsetLeft},0)`}>
            ...
            {series.map((entry) => (
              <g key={entry.key} class={`history-series ${entry.colorClass}`}>
                ...
                {entry.points.map((point) => {
                  const x = getUsageHistoryPointX(point.recordedAtMs, startMs, endMs, chartWidth);
                  const y = Math.round((chartHeight - ((Math.max(0, Math.min(100, point.value)) / 100) * chartHeight)) * 100) / 100;
                  return (
                    <g key={`${entry.key}:${point.sample.recordedAt}`}>
                      <title>{formatUsageHistoryTooltip(point.sample, entry.label, point.value)}</title>
          ...
          {axisLevels.map((level) => {
            const y = Math.round((chartHeight - ((level / 100) * chartHeight)) * 100) / 100;
            return (
              <text key={`axis:${level}`} x={chartOffsetLeft - 8} y={y + 4} text-anchor="end" class="history-chart__axis-label">
                {level}%
              </text>
            );
          })}
          <text
            x="14"
            y={Math.round(chartHeight / 2)}
            transform={`rotate(-90 14 ${Math.round(chartHeight / 2)})`}
            text-anchor="middle"
            class="history-chart__axis-label"
          >
            Used %
          </text>
          <text
            x={chartOffsetLeft + chartWidth / 2}
            y={chartHeight + 24}
            text-anchor="middle"
            class="history-chart__axis-label"
          >
      ...
      <div class="history-card__footer">
        <span>{formatTimestamp(props.samples[0]?.recordedAt ?? '')}</span>
        <span>{formatTimestamp(props.samples[props.samples.length - 1]?.recordedAt ?? '')}</span>
      </div>
      <div class="history-card__legend">
        {series.map((entry) => (
          <span key={`legend:${entry.key}`} class="history-legend">
...
function RecentSamples(props: { samples: UsageHistorySample[] }) {
  ...
  return (
    <div class="sample-table">
      ...
      {rows.map((sample) => {
        ...
        return (
          <div key={sample.recordedAt} class="sample-table__row" title={formatUsageHistoryTooltip(sample, hoverLabel, hoverValue)}>
            <strong>{formatTimestamp(sample.recordedAt)}</strong>
            <span>{primary}</span>
            <span>{secondary}</span>
            <span>{getUsageHistorySourceLabel(sample.sourceFile)}</span>
          </div>
        );
      })}
    </div>
...
function TokenPanel(props: { profile: UsagePanelProfile }) {
  if (!props.profile.snapshot?.totalUsage && !props.profile.snapshot?.lastUsage) {
    return renderEmptyMessage('No token usage yet', 'Prompt Codex on this profile once to populate total and last token totals.');
  }

  return (
    <section class="analytics-panel__section analytics-panel__section--token">
      <div class="analytics-panel__section-head">
        <h4>Token Usage</h4>
        {props.profile.snapshot?.totalUsage
          ? <span class="badge">{formatCompactTokenCount(props.profile.snapshot.totalUsage.totalTokens)} total</span>
          : null}
      </div>
      {props.profile.snapshot?.totalUsage ? <div class="analytics-panel__detail"><strong>Total:</strong> {formatTokenUsage(props.profile.snapshot.totalUsage)}</div> : null}
      {props.profile.snapshot?.lastUsage ? <div class="analytics-panel__detail"><strong>Last:</strong> {formatTokenUsage(props.profile.snapshot.lastUsage)}</div> : null}
...
export function AnalyticsPanel(props: AnalyticsPanelProps) {
  ...
  const samples = getUsageHistorySamplesForRange(props.profile.history, props.state.historyRange);
  ...
  return (
    <article class="analytics-panel">
      ...
      {!descriptors.length ? renderEmptyMessage(
        'Remaining usage is unknown',
        'No live usage windows were found for this profile. Check the refresh status above to see whether the last refresh found nothing newer.'
      ) : descriptors.map((descriptor) => (
        <UsageWindowSection key={descriptor.longLabel} state={props.state} descriptor={descriptor} />
      ))}
      <section class="analytics-panel__section">
        <div class="analytics-panel__section-head">
          <h4>Usage History</h4>
          <span class="badge">{getHistoryRangeLabel(props.state.historyRange)}</span>
        </div>
        {samples.length ? (
          <div class="analytics-panel__history-stack">
            <div class="history-summary">
              <div>
                <span>Latest sample</span>
                <strong>{formatTimestamp(latestSample.recordedAt)}</strong>
              </div>
              <div>
                <span>Source</span>
                <strong>{getUsageHistorySourceLabel(latestSample.sourceFile)}</strong>
              </div>
              <div>
                <span>Context</span>
                <strong>{getUsageHistoryContextLabel(latestSample.sourceFile) ?? 'No extra context'}</strong>
              </div>
              <div>
                <span>5H Used</span>
                <strong>{typeof latestSample.primaryUsedPercent === 'number' ? formatPercentValue(latestSample.primaryUsedPercent) : 'Unknown'}</strong>
              </div>
              <div>
                <span>Weekly Used</span>
                <strong>{typeof latestSample.secondaryUsedPercent === 'number' ? formatPercentValue(latestSample.secondaryUsedPercent) : 'Unknown'}</strong>
              </div>
              <div>
                <span>Total Tokens</span>
                <strong>{latestSample.totalUsage ? formatCompactTokenCount(latestSample.totalUsage.totalTokens) : 'Unknown'}</strong>
              </div>
            </div>
            <HistoryChart state={props.state} samples={samples} />
            <RecentSamples samples={samples} />
          </div>
        ) : renderEmptyMessage('No historical samples yet', 'Remaining usage is unknown until Codex emits fresh usage data for this profile.')}
      </section>
