...
exports.ProfileGrid = ProfileGrid;
...
function ProfileUsageMiniRow(props) {
    ...
    const percent = (0, helpers_1.getDisplayPercentValue)(descriptor.window, state.percentDisplayMode);
    return (<div class="profile-card__metric">
...
function ProfileGrid(props) {
    return (<section class="profile-grid-section">
      ...
      <div class="profile-grid">
        {props.state.profiles.map((profile) => {
            const descriptors = (0, helpers_1.getUsageWindowDescriptors)(profile.snapshot);
            ...
            return (<article class={`profile-card ${profile.isActive ? 'profile-card--active' : ''} ${isCompare ? 'profile-card--compare' : ''}`} key={profile.id}>
              ...
              <div class="profile-card__body">
                {descriptors.length
                    ? descriptors.map((descriptor) => (<ProfileUsageMiniRow key={descriptor.longLabel} state={props.state} profile={profile} descriptor={descriptor}/>))
              ...
              <div class="profile-card__actions">
                {!profile.isActive ? (<button type="button" class="button button--ghost" onClick={() => props.onSwitchProfile(profile.id)} disabled={props.pendingAction === switchAction}>
                    {props.pendingAction === switchAction ? 'Switching…' : 'Switch'}
                  </button>) : <span class="profile-card__action-placeholder">Active profile</span>}
                {!profile.isActive ? (<button type="button" class="button button--secondary" onClick={() => props.onCompareChange(profile.id)}>
                    {isCompare ? 'Comparing' : 'Compare'}
                  </button>) : (<button type="button" class="button button--secondary" disabled>
                    Current
                  </button>)}
              </div>
            </article>);
...
//# sourceMappingURL=profile-grid.js.map
